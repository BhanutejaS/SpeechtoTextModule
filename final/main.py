...
def tts_play_worker():
    """
    Continuous playback worker — keeps OutputStream open permanently.
    Absolutely silent tail (no pops/clicks/thuds).
    """
    ...
    fade_buffer = np.zeros((0,), dtype=np.int16)
    t0_tts = t0_pipeline = None
    ...
    while True:
        try:
            ...
            # feed silence if nothing to play (keeps DAC stable)
        ...
        if isinstance(item, dict) and item.get("type") == "play_end":
            # fade last buffered samples if available
            if fade_buffer.size > 0:
                fade_len = min(int(device_sr * 0.1), len(fade_buffer))
                fade = np.linspace(1, 0, fade_len, dtype=np.float32)
                tail = (fade_buffer[-fade_len:].astype(np.float32) * fade).astype(np.int16)
                stream.write(tail.reshape(-1, 1))
            # play 0.3 s silence to drain
            pad = np.zeros((int(device_sr * 0.3), 1), dtype=np.int16)
            stream.write(pad)
            stream.write(pad)
            # tiny DAC grace
            time.sleep(0.15)
            ...
            MIC_COOLDOWN_UNTIL = time.time() + 1.2
            print("[POST-TTS] Mic cooldown for 1.2 s after playback.")
        ...
        fade_buffer = x[-int(device_sr * 0.1):, 0].copy()

        if not first_audio_emitted and t0_tts is not None:
            sd.stop()  # resets any previous buffering
            now = time.time()
...
def gpt_stream_and_queue_tts(segment_text: str, pipeline_t0: float):
    ...
    with LLM_LOCK:
        ...
        try:
            ...
            BUFFER_TARGET_WORDS = 6       # send to TTS every ~6 words
            MIN_FLUSH_INTERVAL = 0.35     # seconds between TTS sends
            ...
            for chunk in resp:
                ...
                if not sent_start_marker:
                  ...
                  # Flush to TTS every few words or every 0.35s
                now = time.time()
                if (len(clause_buf.split()) >= BUFFER_TARGET_WORDS or
                     any(p in token for p in ".?!") or
                     now - last_flush_time > MIN_FLUSH_INTERVAL):

                     tts_synth_q.put({"type": "text", "text": clause_buf.strip(), "gen_id": my_gen})
                     clause_buf = ""
                     last_flush_time = now

              # Flush leftovers after stream ends
            if clause_buf.strip():
                 tts_synth_q.put({"type": "text", "text": clause_buf.strip(), "gen_id": my_gen})
